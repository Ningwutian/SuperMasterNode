...
const char * const BITCOIN_CONF_FILENAME = "smn.conf";
const char * const BITCOIN_PID_FILENAME = "smnd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "smn" is a composite category enabling all SuperMasterNode-related debug output
            if(ptrCategory->count(string("smn"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SMN
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SMN
    // Mac: ~/Library/Application Support/SMN
    // Unix: ~/.smn
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SMN";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SMN";
#else
    // Unix
    return pathRet / ".smn";
#endif
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty smn.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override smn.conf
